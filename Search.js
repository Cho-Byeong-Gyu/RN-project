    ...
    async getHouseListData() {                      // axios를 활용한 api통신을 통해 서버로부터 숙소 리스트들을 불러오는 함수
        try{
            ...
            const data = response.data.map(item => ({
                ...
                imageUrl: houseIMG1, // 예시로 하나의 이미지만 사용, 필요하다면 수정
                reviewScore: "못 받음", // 서버 데이터에 따라 수정 필요
                reviewCount: "못 받음", // 서버 데이터에 따라 수정 필요
                favoriteState: false, // 초기 상태값
                reservationState: false, // 초기 상태값
                clearReservation: false // 초기 상태값
            }));
